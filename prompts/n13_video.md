...
**额度先给骨架，剩下的才是补图的。** 骨架那几张不是「最小充分集」里
可以裁掉的部分 —— 裁掉一张就是那段时间没人告诉模型发生了什么。
要裁只裁补图。

如果骨架张数本身就超过了上限（比如这一段有 4 张 Sheet 而模型只吃 3 张），
**那不是让你砍骨架，是这一段的结构和这个模型对不上**：
在 `capability_note` 里写清「骨架 N 张 > 上限 M 张」，
往回让第十二环节把这一段合并成 M 张以内（合并的是承载颗粒度，不是时间覆盖），
或者换一个吃得下的模型。砍掉一张然后照做，画面会在缺的那一段自己编。
...
**骨架有几张就写几条，一张都不许少。** 一个 SEG 的故事板是 1..N 张有序
Sheet（第十二环节实际产出几张，这里就是几条），编号从 `Image 1` 连续排、
顺序等于 `order`，补图排在骨架之后。

> **这一条以前只给了一张的写法，是个真问题。** 第十二环节按 V6.2 出了
> 2 张 Sheet，而这里只示范 `Image 1 = SB_..._{{SEGMENT}}` 一条 ——
> 于是提示词里只有第一张的身份说明，程序却按骨架顺序把 2 张都传了上去。
> 结果：第二张没有任何身份说明，编号还整体错位一位（提示词说 Image 2 是
> 补图，实际传的是骨架第二张）。**两边都不报错**，出来的画面用错了参考。
> 每一张都要有自己那一条。

每一张 Sheet 各写一条（下面是 2 张的例子，实际有几张写几条）：

```
Image 1 = SBPKG_{{SEGMENT}}_SHEET_A 本段故事板第 1 张（前段）
  是谁/是什么 + 画面可见内容：本段前半的有序关键帧网格，可见 C001 与 C005，
    KF01 C001 坐于床沿、C005 立于床尾，KF03 起身后两人面对面
  故事时间 / 当前状态：本段 0-12 秒，从起始稳定状态到起身完成
  有权控制：这一段时间内关键帧的视觉状态、人物与空间组合、机位、
    动作顺序、道具外观与实例绑定、数量、时间状态、转场机制与锚点
  无权控制：故事板的网格、边框、格子编号、排版；
    自然的中间动作、呼吸微表情、衣料细微物理；**本段后半的内容**
  适用范围：本段 0-12 秒的镜头与转场
  MUST TRANSFORM：把静态关键帧变成连续表演

Image 2 = SBPKG_{{SEGMENT}}_SHEET_B 本段故事板第 2 张（后段）
  是谁/是什么 + 画面可见内容：本段后半的有序关键帧网格，KF04 两人对峙、
    KF06 C005 转身离开
  故事时间 / 当前状态：本段 12-30 秒，从对峙到离场
  有权控制：这一段时间内的视觉状态、组合、机位、动作顺序、转场锚点
  无权控制：网格与排版；**本段前半的内容**
  适用范围：本段 12-30 秒的镜头与转场
  MUST TRANSFORM：把静态关键帧变成连续表演
```

**每张的「适用范围」必须写它自己那段绝对时间**，不许写「本段全程」——
两张都写全程，等于告诉模型这两张说的是同一段时间，
它会把两个状态抹平成一个，或者让后段的画面提前出现。
