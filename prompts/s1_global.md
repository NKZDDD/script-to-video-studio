...
## 每集切几段（`segments`，和集边界一起定）

**你是唯一看得到全篇的环节，所以每集多长必须在这里定死，不能交给后面每集自己猜。**
第二环节只负责按你给的段数去划边界，不再自己判断该切几段——那会导致同样体量的两集
一个切 12 段、一个切 4 段，成片时长差三倍。

一段 = **{{DURATION}} 秒 + 一个明确完成的剧情任务**（角色发现秘密／关键道具出现／
人物遭遇危机／关系反转／状态不可逆改变……）。所以段数由这一集**有多少个这样的任务**
决定，不是按字数机械换算。

定段数时按顺序考虑：

1. **数这一集有几个必须保留的剧情事件**——这是下限，一个事件至少一段，
   不能把两个不可逆事件塞进同一段。
2. **看正文体量**：正文每 120–180 字大致承载一段。字多事件少 → 说明有大段描写，
   按事件数走；字少事件密 → 按事件数走，别为了凑时长注水。
3. **和项目参数里的「单集分钟」对一下**：那是期望值，不是硬指标。
   偏离超过 ±40% 时，在 `pacing_note` 里说明为什么（比如「本集是过渡集，
   只有 5 个事件，硬凑 12 段会注水」）。
4. 同一部剧各集之间**不要求整齐**：剧情重的集自然长，过渡集自然短。
   但不要出现 3 段和 30 段并存这种极端差距，那通常说明集边界切错了。

段数区间：**最少 4 段，最多 40 段**。超出这个范围先回头检查集边界是不是切错了。

> 每集时长 = `segments` × {{DURATION}} 秒。所以给 12 段就是 3 分钟，给 20 段就是 5 分钟。
...
```json
{
  ...
  "episode_ranges": [
    {"episode": "EP01", "title": "本集标题（没有就填空）",
     ...
     "segments": 12,
     "pacing_note": "为什么是这个段数（事件数 / 体量 / 和单集分钟的偏差理由）"}
  ],
  "visual_tone": {
