...
- **`reference_assets` 是「出图时实际要喂的参考图列表」，不是逻辑依赖清单。**
  里面每一项都必须是 `assets[]` 里**会出图**的编号 —— 出图那一步会按这个列表
  逐个去找对应的 png 文件。
  ...
  | 不许写 | 它该去哪 |
  |---|---|
  | 空间母资产 `SP001` | `space_master_id` 那一栏 |
  | 区域编号 `A` / `B` | `space_region_id` 那一栏 |
  | 判成不生产（`decision: skip`）的资产 | 哪儿都不写 |
  | `space_masters` / `characters_in_space` 里出现的任何编号 | 各自的那一栏 |

  一个空间要作为参考图出现时，引的是它 `regions[].environment_asset_id`
  指向的那个环境资产（例如 `S001`），**不是空间编号本身**。

  写错的后果不是报错：出图那一步找不到 `SP001.png`，这一条资产**永远出不来**，
  而引用到它的故事板会跟着停。
...
- **一个 SEG 里有闪回、回忆、穿越时，同一个人可以有多条记录** ——现实里一条、回忆里一条，用 `timeline` 区分（如「现实」「四年前回忆」「穿越前」）。没有闪回就统一填「现实」。
  **同一个 `timeline` 里，一个人在一个 SEG 只能有一条记录** —— 同一时间线上一个人不可能同时在两个地方，写两条下游不知道该信哪个。
  反过来也别为了绕开这条限制，把回忆和现实塞进同一条的 `position` 里：那样下游读不出来这一段其实有两个空间。
  ...
  "assets": [
    {
      ...
      "reference_assets": ["状态资产填写父资产及全部依赖资产；**只能填 assets[] 里会出图的编号**；其他资产填空数组"],
