...
def _truncated(body: str, n: int) -> "LLMError":
    """输出写到一半断了。和「格式不对」是两回事，报错必须分开。

    **可重试，不是 Fatal。** 我一开始做成 LLMFatal，理由是「同一个提示词
    重试多半还是同样的结果」—— 实跑数据推翻了它：同一个模型、同一步、
    同一份剧本，一次断在 8570 token，另一次 16251 token 就成功了。
    是随机的，不重试等于白扔掉那个机会。

    这段话同时是**给模型的反馈**（json_call 会把它附在下一次的提示词后面），
    所以第一句要让模型知道该怎么改：压缩，而不是重写。
    用户该做什么写在 diagnose 的 LLM_TRUNCATED 那一条里，不塞进这里 ——
    塞进来模型会把它当成任务要求。
    """
    tail = re.sub(r"\s+", " ", body[-100:])
    return LLMError(
        f"上一次的输出**在写到一半时被截断了**（收到 {n} 字，JSON 始终没有闭合，"
        f"停在「…{tail}」）。不是格式错，是太长了没写完。\n"
        f"请重新输出一份更紧凑的 JSON。**压描述，不压结构**：\n"
        f"· 可以压：形容词、复述剧本原文、解释理由、同义的重复表述\n"
        f"· 不许压：字段数、条目数，以及模板要求的**逐项分行**格式 —— "
        f"该分成几行标签的就分几行，不许合并成一段话。"
        f"合并之后下游没法逐项校验，会被整批拦下来重做，比截断更贵。")
