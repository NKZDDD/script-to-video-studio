...
def medium_rule(pj: Project) -> str:
    """「视频类型 + 视觉风格」一行。**原样给出去，不替用户丰富提示词。**

    以前 `visual_medium` 只作为【项目基础信息】里的一行值出现，没有任何模板
    把它当约束 —— 实跑选了真人写实，出来的资产提示词写着「高质量3D漫剧风格、
    精致影视级角色建模」，而且不报错。所以它得像字幕、旁白那样每次调用都发。

    `visual_style` 踩的是**同一个坑**（2026-08-23 实跑）：它到今天也没有
    任何模板单独引用，只走 brief 里的一行 —— 用户填「2.5D动漫风格」，
    拍成什么形式选「3D漫剧」，资产提示词里只出现「3D漫剧电影感」，
    2.5D 一个字都没有。修法和 medium 当年一样：提到全局原则这条强位置，
    而且**和视频类型拼在同一条里** —— 两项本来就是一对（媒介管怎么拍、
    风格管什么样貌），拆成两条弱化的是「它们要一起生效」这件事。
    ...
    out = f"视频类型：{MEDIUM_ZH.get(raw) or raw or '真人短剧'}"
    style = str(v.get("visual_style") or "").strip()
    if style:
        out += f"；视觉风格：{style}"
    return out
