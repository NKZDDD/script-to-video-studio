...
STAGES = [
    ...
    {"id": "d2", "no": 16, "name": "排序拼接与交付", "kind": "local",
     "scope": "episode", "out": ""},
    # 字幕排在拼接**之后**，因为它加在成片上，不是加在分段视频上。
    # 逐段加的话，段与段之间的一句话会被切成两半，而且每段都要单独识别一次。
    #
    # 它是**本体系里唯一一个整环节外包**的步骤：识别、断句、优化、压制
    # 全交给 VideoCaptioner（见 core/subtitle.py）。所以 kind 是 local
    # 而不是 llm —— 就算它内部调了 LLM，那也是它的调用、它的 key、
    # 它的重试，本程序的分析引擎那套完全不参与。混成 llm 的话，
    # 「至少几次分析调用」会把它算进去，那个数就成了假的。
    {"id": "d3", "no": 17, "name": "字幕", "kind": "local",
     "scope": "episode", "out": ""},
]
