...
CATALOG = {
    ...
    "LLM_TRUNCATED": {
        ...
        "why": "这一步要写的内容太长，写到一半被切断，所以后面的步骤读不出完整结果。"
               ...
               "再加每一集的边界，很容易超。"
               "有两种切法，改法完全不同："
               "① 模型自己到了输出上限，报错里会带 max_tokens 或 length；"
               "② 中转站把长响应切了 —— 这种没有任何标记，表现是 JSON 停在句子中间、"
               "围栏没闭合，调大上限完全没用。",
        "where": "设置 → 分析引擎（输出上限、流式输出）",
        "fix": ["报错里带 max_tokens 或 length 的：把「单次输出上限」调大，"
                "比如 16000 改成 32000 或 64000",
                "报错里说「JSON 一直没有闭合」的：那是中转站切的，调上限没用 —— "
                "先把「流式输出」关掉再试一次",
                "还截断就换一个模型；或者用「只测第一集」，"
                "让这一步要产出的东西少一些",
                ...
                "注意：这一条不会自动重试。同一个提示词重试多半同样被截断，"
                "只是把钱花三倍，所以程序直接停下来等你处理"],
