...
CATALOG = {
    ...
    "REF_MISSING": {
        ...
        "why": "这一步声明了上游图片作为参考，但对应文件现在缺失或不完整。"
               "可能是还没做，也可能是被删了。",
        "where": "「生产」页的执行顺序",
        "fix": ["按本条任务列出的参考图依赖补齐上游图片，再运行当前任务",
    ...
    "RESULT_DOWNLOAD": {
        "title": "服务商已返回结果，但取回或文件检查失败",
        "why": "生成结果已经返回，但下载没有完成，或取回的图片无法完整解码。"
               "有结果链接时只重取同一链接；内嵌结果无法重取。"
               "程序已停止自动重新生成，避免重复计费。",
        "where": "原任务结果、错误详情中的下载地址和目标路径",
        "fix": ["先打开下面的结果地址，或去服务商原任务页下载已经生成的文件",
                "确认取回的文件完整可用，再放到下面的目标路径；链接失效时向服务商索取原任务的新下载链接",
                "文件补回前不要重新提交生成任务"],
        "resume": "把原任务结果放回目标路径，再继续生产；已有文件会被跳过。",
        "resumable": True, "scope": "task", "level": "error",
    },
    "TIMEOUT": {
        "title": "等太久了还没出结果",
