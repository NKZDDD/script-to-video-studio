...
CATALOG = {
    ...
    "NOTHING_TO_ASSEMBLE": {
        ...
        "why": "拼接只是把已经做好的分段视频按段号接起来。一段视频都没有，"
               ...
               "视频生产被闸门拦住了（审计 BLOCK、覆盖、道具对账、位置状态"
               "任意一道），或者那一批视频任务本身没跑成。"
               "闸门拦住时上面几步会显示「被拦」，最后这一步只会说拼不了，"
               "很容易让人以为是拼接坏了，去查 ffmpeg —— 那是白查。",
        "where": "先看「生产」页上面的四道闸门有没有红的；再看「产物」页有没有分段视频",
        "fix": ["有闸门是红的：先按它列的问题去改，或者在闸门面板上写理由显式放行",
                "闸门都绿的：说明视频生产那一批没跑成，去「任务明细」看那几条的报错",
