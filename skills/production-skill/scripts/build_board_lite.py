# -*- coding: utf-8 -*-
"""build_board_lite.py — V7.0 轻量离线分镜看板（兼容历史输入）
...
看板区块（与 6.8.3 同构，砍掉 N1~N11 检查层 / validate 依赖 / json 快照）:
    ...
    分镜生产层: ① 时间轴泳道 · ② 镜头卡矩阵(拆解) · ③ 投喂提示词原文(成品)
                ④ 资产总表(P2) · ⑤ 资产出图提示词(P2.5·按输入提供) · ⑥ 发布文案(P6·可选)
...
轻量铁律（简化自 6.8.3 K1~K7，只留 4 条）:
    ...
    K-lite-4  已有资产出图词与视频投喂词完整呈现；没有P2.5不强制补产资产
"""
...
ORDINAL_RE = re.compile(r"^\[镜头\s*(\d+)\]\s*(.*)$")


def untimed_shot(line):
    m = ORDINAL_RE.match(line.strip())
    if m:
        return {"no": int(m.group(1)), "start": None, "end": None, "text": m.group(2)}
    return None


def segment_duration(seg):
    fields = seg.get("fields", {})
    for key in ("时长", "目标时长", "单段时长", "SEG时长"):
        m = re.fullmatch(r"\s*(\d+(?:\.\d+)?)\s*(?:秒|s)?\s*", fields.get(key, ""), re.I)
        if m:
            return float(m.group(1))
    shots = seg.get("shots", [])
    if shots and all(s["end"] is not None for s in shots):
        return max(s["end"] for s in shots)
    return None


VIDEO_SECTIONS = ("画幅风格", "参考图", "站位声明", "时间轴分镜", "接续状态", "音效", "强制禁止项")
FIELD_RE = re.compile(r"^【([^】]{2,8})】\s*[:：]?\s*(.*)$")


def prompt_sections(raw):
    """Read the seven current headings without rewriting the source prompt."""
    result, current = {}, None
    for line in raw.splitlines():
        match = FIELD_RE.match(line.strip())
        if match and match.group(1) in VIDEO_SECTIONS:
            current = match.group(1)
            result.setdefault(current, []).append(match.group(2))
        elif current:
...
def parse_md(md_text: str) -> dict:
    ...
    def flush_plain():
        ...
        if not section:
            ...
            # Notes outside asset/video fences are still source material.
            _extra_block(section, {"t": "p", "v": text})
    ...
    def flush_fence():
        ...
        if fence_owner is not None:
            fence_owner.setdefault("blocks", []).append(text)
        else:
            _extra_block(fence_title or "（卷首说明）", {"t": "code", "v": text})
        fence_lines = []
    ...
    for raw in lines:
        ...
        if s.startswith("```"):
            if in_fence:
                ...
                flush_plain()
                in_fence = True
                fence_owner = ap if ap is not None else seg
                fence_title = section
            continue
        if in_fence:
        ...
        if m_sub:
            ...
            if re.match(r"^段\s*\d", sub_head) or re.match(r"^U\d+\b", sub_head) \
               or re.match(r"^第\s*\d+\s*段", sub_head):
                ...
                ap = None
                seg = None
                _extra_block(section or "（卷首说明）", {"t": "p", "v": line})
            continue
        ...
        if s.startswith("|") and section:
            ...
            if rows:
                ...
                if z in ("script", ""):
                    ...
                    flush_plain()
                    _extra_block(section, {"t": "row", "cells": cells})
                continue
        ...
        if seg is not None:
            ...
            if fmt:
                ...
                continue
            ordinal = untimed_shot(s)
            if ordinal:
                seg["shots"].append(ordinal)
                continue
        plain.append(line)

    if in_fence:
        flush_fence()
        data["warnings"].append({"seg": 0, "msg": "未闭合代码围栏：已保留正文，请检查源文件"})
    flush_tbl()
    flush_plain()

    # Ownership is captured while parsing, never inferred from title prefixes.
    # All fenced blocks are retained in source order, including short blocks.
    for seg in data["segments"]:
        seg["raw"] = "\n\n".join(seg.get("blocks", []))
        seg["sections"] = prompt_sections(seg["raw"])
    ...
    for seg in data["segments"]:
        ...
        if not seg["shots"]:
            ...
            for l in seg["raw"].splitlines():     # K-lite-2 降级：从围栏内提取时间戳
                if re.match(r"^\s*【[^】]+】", l):
                    current_shot = None
                ordinal = untimed_shot(l)
                if ordinal:
                    seg["shots"].append(ordinal)
                    current_shot = ordinal
                    continue
                if current_shot is not None and re.match(
                        r"^\s*(?:画面|发声角色|对白原文|内心独白原文|口型与反应|环境与动作声|配乐)\s*[:：]", l):
                    current_shot["text"] = (current_shot["text"] + "\n" + l.strip()).lstrip("\n")
                    continue
        ...
        if not seg["shots"]:
            seg["warnings"].append("未解析到 [镜头N] 逐镜描述（兼容历史时间码）")
        else:
            prev = None
            for sh in seg["shots"]:
    ...
    for seg in data["segments"]:
        ...
        off = off + duration if off is not None and duration is not None else None

    # Only exact explicit bindings are direct references; mentions/prefixes are not.
    direct_refs = {
        seg['no']: set(re.findall(r'Image\s+\d+\s*=\s*([^\s,，;；。`]+)', seg['raw']))
        for seg in data['segments']
    }
    for a in data["assets"]:
        code = a.get("code", "").strip().strip('`').lstrip('@')
        a["refs"] = [seg["no"] for seg in data["segments"] if code and code in direct_refs[seg['no']]]
    return data


def total_duration(data):
    """无镜头时间时读取明确的段总时长；缺失则返回未知，不均分或编造。"""
    segs = data["segments"]
    if not segs:
        return 0
    if any(not s["shots"] or any(sh["start"] is None for sh in s["shots"]) for s in segs):
        values = [segment_duration(s) for s in segs]
        return sum(values) if all(v is not None for v in values) else None
    starts = [s["shots"][0]["start"] for s in segs]
    if starts and all(abs(x - starts[0]) < 0.01 for x in starts) and starts[0] == 0:
        return sum(segment_duration(s) for s in segs)
...
def build_html(d: dict) -> str:
    ...
    # A literal closing script tag in a prompt must remain data, not terminate JS.
    return TEMPLATE.replace("@@DATA@@", json.dumps(payload, ensure_ascii=False).replace("<", "\\u003c")) \
...
<h2>⑤ 资产出图提示词 <span class="sub">（生成人物 / 场景 / 道具资产图用，可整段复制；按本次输入提供）</span></h2><div id="aprompts"></div>
...
function renderBlocks(bl){
  ...
  (bl||[]).forEach(b=>{
    ...
    else { flushT(); out+=(b.t==='code'?'<pre>':'<div class="bd">')+esc(b.v)+(b.t==='code'?'</pre>':'</div>'); }
  });
  flushT();
  return out;
}
...
function hdr(){
  ...
  document.getElementById('hdr').textContent =
    `模型 ${p.model} · 画幅 ${p.aspect} · ${p.seg_count} 段 / ${p.shot_count} 镜 / 总时长 ${p.total == null ? "未知" : p.total+"s"} · 编译于 ${p.built_at}`;
...
function paint(){
  ...
  document.getElementById('lanes').innerHTML = D.segments.map(s=>{
    const cls = ['','s1','s2','s3'];
    if (s.off == null || s.shots.some(sh=>sh.start == null || sh.end == null)) {
      const blocks = s.shots.map(sh=>'<span class="chip">镜'+sh.no+'</span>').join(' → ');
      return '<div class="lane '+(vis(s.no)?'':'hide')+'"><div class="nm">'+esc(s.name)+' · 镜头顺序（未指定逐镜时间）</div><div>'+blocks+'</div></div>';
    }
    const off = s.off||0;
  ...
  document.getElementById('cards').innerHTML = D.segments.filter(s=>vis(s.no)).flatMap(s=>
    s.shots.map(sh=>`<div class="card"><div class="hd"><span>${esc(s.name)} · 镜${sh.no}</span>
      <span class="ph">${sh.start == null || sh.end == null ? "按镜头顺序" : `[${sh.start}-${sh.end}s] · ${Math.max(sh.end-sh.start,0).toFixed(1).replace(/\.0$/,'')}s`}</span></div>
  ...
  // ⑤ 资产出图提示词（按输入提供）；同一场景资产的连排条目（①②③）聚合为一张卡
  const _grp=[];
  ...
  document.getElementById('aprompts').innerHTML = _grp.length
    ...
    : '<div class="f">本次输入未提供资产出图提示词；是否需要新资产由生产范围与引用完整性决定。</div>';
  // ⑥ 发布文案
...
def main():
    ...
    duration_label = f"{duration:g}s" if duration is not None else "未知"
    print(f"[+] 总时长：{duration_label} · WARN：{len(data['warnings'])}")
